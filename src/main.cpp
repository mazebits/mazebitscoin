...
#if defined(NDEBUG)
# error "Mazebits cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc00d08c6245b3821a72952d7efab7847d40de6b40d92f0479b5b89f06a4c8dc7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mazebits: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mazebits Signed Message:\n";
...
{
    // Mazebits: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight > 700 && nHeight < 50000){
      nSubsidy = 0 * COIN;
    }
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Mazebits: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Mazebits: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Mazebits: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mazebits: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mazebits: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc00d08c6245b3821a72952d7efab7847d40de6b40d92f0479b5b89f06a4c8dc7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is mazebits Developement. Thank you";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("412384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516856475;
        ...
        assert(block.hashMerkleRoot == uint256("0x5626dcf34bcefdbe73a8c0dcf96578c0b7f50964189a2aea7c3f4f44c2d418b3"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Mazebits: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MazebitsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MazebitsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MazebitsMiner(CWallet *pwallet)
{
    printf("MazebitsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mazebits-miner");
    ...
    try { loop {
        ...
        printf("Running MazebitsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MazebitsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MazebitsMiner, pwallet));
}
